...
func (s *WebSocketServer) handleMCPWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	log.Infof("Received WebSocket connection request from MCP server, Agent ID: %s", agentId)
	...
	go func() {
		<-mcpClient.Ctx.Done()
		log.Infof("MCP connection for server %s has been disconnected", mcpClient.GetServerName())
	}()

	log.Infof("MCP connection for server %s has been established", mcpClient.GetServerName()) // todo
}
...
func (s *WebSocketServer) parseMCPToken(tokenString string) (*MCPClaims, error) {
	...
	// Use the same key as token generation
