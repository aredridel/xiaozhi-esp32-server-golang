...
// WebSocketConn implements types.IConn interface, adapting WebSocket connections
...
func NewWebSocketConn(conn *websocket.Conn, deviceID string, isMqttUdpBridge bool) *WebSocketConn {
	...
	conn.SetPongHandler(func(appData string) error {
		log.Debugf("Received pong message, device ID: %s", deviceID)
		return nil
	})

	// Start heartbeat detection goroutine
	go func() {
		...
		for {
			...
			case <-ticker.C:
				if err := instance.conn.WriteControl(websocket.PingMessage, []byte{}, time.Now().Add(5*time.Second)); err != nil {
					log.Errorf("Failed to send ping message, device ID: %s, error: %v", deviceID, err)
				...
				log.Debugf("Ping message sent successfully, device ID: %s", deviceID)
		...
		for {
			...
			default:
				...
				if err != nil {
					...
					for _, cb := range instance.onCloseCbList {
						cb(instance.deviceID) // Notify registrant to exit
					}
					return
				}
...
// Adapt mqtt udp bridge data format
...
func (c *WebSocketConn) tryUnpackUdpBridgeAudioPacket(buffer []byte) []byte {
	...
	// Check if first 8 bytes are all 0
	for i := 0; i < 8; i++ {
		if buffer[i] != 0 {
			return buffer
		}
...
func (c *WebSocketConn) packUdpBridgeAudioPacket(buffer []byte) []byte {
	header := make([]byte, 16)
	// First 8 bytes are all 0, already initialized
	// Bytes 9~12 write current timestamp (seconds)
	timestamp := uint32(time.Now().Unix())
	binary.BigEndian.PutUint32(header[8:12], timestamp)
	// Bytes 13~16 write audio length
