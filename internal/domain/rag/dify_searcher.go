...
func (s *difySearcher) Search(
	...
	if len(errs) > 0 {
		log.Warnf("Dify knowledgelibraryretrievepartfailed: %s", strings.Join(errs, "; "))
	}
	return ret, nil
}
...
func (s *difySearcher) searchOneDataset(
	...
	if err != nil {
		return nil, fmt.Errorf("createDifyrequestfailed(dataset_id=%s): %w", datasetID, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("callDifyfailed(dataset_id=%s): %w", datasetID, err)
	}
	if resp.StatusCode >= 400 {
		bodyBytes, _ := io.ReadAll(resp.Body)
		resp.Body.Close()
		return nil, fmt.Errorf("Difyreturnabnormal(dataset_id=%s): %d %s", datasetID, resp.StatusCode, string(bodyBytes))
	}
	...
	resp.Body.Close()
	if decodeErr != nil {
		return nil, fmt.Errorf("parseDifyreturnfailed(dataset_id=%s): %w", datasetID, decodeErr)
	}
