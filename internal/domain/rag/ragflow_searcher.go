...
func (s *ragflowSearcher) Search(
	...
	if len(errs) > 0 {
		log.Warnf("RAGFlow knowledgelibraryretrievepartfailed: %s", strings.Join(errs, "; "))
	}
	return ret, nil
}
...
func (s *ragflowSearcher) searchOneDataset(
	...
	if err != nil {
		return nil, fmt.Errorf("createRAGFlowrequestfailed(dataset_id=%s): %w", datasetID, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("callRAGFlowfailed(dataset_id=%s): %w", datasetID, err)
	}

	bodyBytes, _ := io.ReadAll(resp.Body)
	resp.Body.Close()
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("RAGFlowreturnabnormal(dataset_id=%s): %d %s", datasetID, resp.StatusCode, string(bodyBytes))
	}
	...
	if err := json.Unmarshal(bodyBytes, &ragflowResp); err != nil {
		return nil, fmt.Errorf("parseRAGFlowreturnfailed(dataset_id=%s): %w", datasetID, err)
	}
	if ragflowResp.Code != 0 {
		return nil, fmt.Errorf("RAGFlowrequestfailed(dataset_id=%s): code=%d message=%s", datasetID, ragflowResp.Code, strings.TrimSpace(ragflowResp.Message))
	}
