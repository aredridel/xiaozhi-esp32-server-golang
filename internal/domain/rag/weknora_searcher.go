...
func (s *weknoraSearcher) Search(
	...
	if len(errs) > 0 {
		log.Warnf("Weknora knowledgelibraryretrievepartfailed: %s", strings.Join(errs, "; "))
	}
	return ret, nil
}
...
func (s *weknoraSearcher) searchOneKnowledgeBase(
	...
	if err != nil {
		return nil, fmt.Errorf("createWeknorarequestfailed(knowledge_base_id=%s): %w", datasetID, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("callWeknorafailed(knowledge_base_id=%s): %w", datasetID, err)
	}

	bodyBytes, _ := io.ReadAll(resp.Body)
	resp.Body.Close()
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("Weknorareturnabnormal(knowledge_base_id=%s): %d %s", datasetID, resp.StatusCode, string(bodyBytes))
	}
	...
	if err := json.Unmarshal(bodyBytes, &weknoraResp); err != nil {
		return nil, fmt.Errorf("parseWeknorareturnfailed(knowledge_base_id=%s): %w", datasetID, err)
	}
	if weknoraResp.Success != nil && !*weknoraResp.Success {
		...
		return nil, fmt.Errorf("Weknorarequestfailed(knowledge_base_id=%s): %s", datasetID, msg)
	}
	if code, ok := parseCodeInt(weknoraResp.Code); ok && code != 0 {
		...
		return nil, fmt.Errorf("Weknorarequestfailed(knowledge_base_id=%s): code=%d message=%s", datasetID, code, msg)
	}
