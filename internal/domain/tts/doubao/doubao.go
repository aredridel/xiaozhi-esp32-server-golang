...
func (p *DoubaoTTSProvider) streamHTTP(ctx context.Context, text string, sampleRate int, frameDuration int) (chan []byte, error) {
	...
	if voice == "" {
		return nil, fmt.Errorf("doubao TTS missing voice")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("serialize doubao TTS request failed: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, p.APIURL, bytes.NewReader(bodyBytes))
	if err != nil {
		return nil, fmt.Errorf("create doubao TTS request failed: %w", err)
	}
	...
	go func() {
		...
		if resp.StatusCode >= http.StatusBadRequest {
			...
			log.Errorf("doubao HTTP TTS returned error: status=%d body=%s", resp.StatusCode, strings.TrimSpace(string(body)))
			close(outputChan)
			return
		}
...
func (p *DoubaoTTSProvider) decodeStreamResponse(ctx context.Context, body io.Reader, outputChan chan []byte, sampleRate int, frameDuration int) {
	...
	if err != nil {
		log.Errorf("create doubao audio decoder failed: %v", err)
		_ = pipeReader.Close()
		_ = pipeWriter.Close()
		close(outputChan)
		return
...
func parseDoubaoTTSStreamEvent(raw []byte) (*doubaoTTSV3Event, error) {
	...
	if err := json.Unmarshal([]byte(line), &event); err != nil {
		return nil, fmt.Errorf("parse doubao TTS streaming event failed: %w", err)
	}
	if event.Code != 0 {
		msg := strings.TrimSpace(event.Message)
		if msg == "" {
			msg = fmt.Sprintf("doubao TTS returned error code %d", event.Code)
		}
...
func decodeDoubaoTTSAudioChunk(event *doubaoTTSV3Event) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("decode doubao audio data failed: %w", err)
	}
	return chunk, nil
}
