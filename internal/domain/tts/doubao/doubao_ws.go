...
func (p *DoubaoWSProvider) TextToSpeech(ctx context.Context, text string, sampleRate int, channels int, frameDuration int) ([][]byte, error) {
	...
	if len(frames) == 0 {
		return nil, fmt.Errorf("Doubao WebSocket TTS returned empty audio")
	}
	return frames, nil
}
...
func (p *DoubaoWSProvider) TextToSpeechStream(ctx context.Context, text string, sampleRate int, channels int, frameDuration int) (chan []byte, error) {
	...
	for idx, candidate := range tryResolved {
		...
		log.Warnf("Doubao WebSocket TTS resource family mismatch, trying fallback: voice=%s from=%s to=%s", voice, candidate.ResourceID, tryResolved[idx+1].ResourceID)
	}

	return nil, fmt.Errorf("Doubao WebSocket TTS could not find available resource family")
}
...
func (p *DoubaoWSProvider) textToSpeechStreamWithModel(ctx context.Context, text string, sampleRate int, frameDuration int, resolved resolvedTTSModel) (chan []byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("build Doubao WebSocket TTS request failed: %w", err)
	}
	...
	if err != nil {
		...
		_ = pipeWriter.Close()
		close(outputChan)
		return nil, fmt.Errorf("create Doubao WebSocket audio decoder failed: %w", err)
	}
	go func() {
	...
	go func() {
		...
		if err := conn.WriteMessage(websocket.BinaryMessage, requestFrame); err != nil {
			trySendDoubaoWSAttemptResult(attemptResult, doubaoWSAttemptResult{err: fmt.Errorf("send Doubao WebSocket TTS request failed: %w", err)})
			_ = pipeWriter.CloseWithError(fmt.Errorf("send Doubao WebSocket TTS request failed: %w", err))
			return
		}
		...
		for {
			...
			if err != nil {
				...
				trySendDoubaoWSAttemptResult(attemptResult, doubaoWSAttemptResult{err: fmt.Errorf("read Doubao WebSocket TTS response failed: %w", err)})
				_ = pipeWriter.CloseWithError(fmt.Errorf("read Doubao WebSocket TTS response failed: %w", err))
				return
			}
...
func buildDoubaoWSBinaryRequest(req any) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("serialize Doubao WebSocket TTS request failed: %w", err)
	}
	compressed, err := gzipCompressDoubao(payload)
	if err != nil {
		return nil, fmt.Errorf("compress Doubao WebSocket TTS request failed: %w", err)
	}
...
func parseDoubaoWSBinaryResponse(frame []byte) ([]byte, bool, error) {
	if len(frame) < 4 {
		return nil, false, fmt.Errorf("parse Doubao WebSocket TTS response failed: response frame length insufficient")
	}

	headerSizeBytes := int(frame[0]&0x0f) * 4
	if headerSizeBytes <= 0 || len(frame) < headerSizeBytes {
		return nil, false, fmt.Errorf("parse Doubao WebSocket TTS response failed: illegal header length")
	}
	...
	case 0x0b:
		...
		if len(payload) < 8 {
			return nil, false, fmt.Errorf("parse Doubao WebSocket audio frame failed: payload length insufficient")
		}
	...
	case 0x0c:
		if len(payload) < 4 {
			return nil, false, fmt.Errorf("parse Doubao WebSocket pre-endpoint message failed: payload length insufficient")
		}
		...
		if compression == 1 {
			...
			if err != nil {
				return nil, false, fmt.Errorf("decompress Doubao WebSocket pre-endpoint message failed: %w", err)
			}
		}
	...
	case 0x0f:
		if len(payload) < 8 {
			return nil, false, fmt.Errorf("parse Doubao WebSocket error frame failed: payload length insufficient")
		}
		...
		if compression == 1 {
			...
			if err != nil {
				return nil, false, fmt.Errorf("decompress Doubao WebSocket error frame failed: %w", err)
			}
		}
		msg := strings.TrimSpace(string(errPayload))
		if msg == "" {
			msg = fmt.Sprintf("Doubao WebSocket TTS server-side error (code=%d)", code)
		}
	...
	default:
		...
		return nil, false, fmt.Errorf("parse Doubao WebSocket TTS response failed: unknown message type %d, first packet=%x", messageType, preview)
	}
}
...
func parseDoubaoWSFullServerResponse(payload []byte, compression byte, flags byte) ([]byte, bool, error) {
	...
	if len(payload) < 8 {
		return nil, false, fmt.Errorf("parse Doubao WebSocket ACK frame failed: payload length insufficient")
	}
	...
	if compression == 1 {
		...
		if err != nil {
			return nil, false, fmt.Errorf("decompress Doubao WebSocket ACK frame failed: %w", err)
		}
	}
	...
	if err := json.Unmarshal(body, &event); err != nil {
		return nil, false, fmt.Errorf("parse Doubao WebSocket ACK JSON failed: %w", err)
	}
...
func extractDoubaoWSFlag4Payload(payload []byte) (int32, string, []byte, error) {
	if len(payload) < 12 {
		return 0, "", nil, fmt.Errorf("parse Doubao WebSocket flags=4 frame failed: payload length insufficient")
	}
	...
	if requestIDLen < 0 || len(payload) < 8+requestIDLen+4 {
		return 0, "", nil, fmt.Errorf("parse Doubao WebSocket flags=4 frame failed: request_id length illegal")
	}
...
func formatDoubaoWSConnectError(err error, resp *http.Response) error {
	if resp == nil {
		return fmt.Errorf("establish Doubao WebSocket TTS connection failed: %w", err)
	}
	...
	if readErr != nil && bodyText == "" {
		return fmt.Errorf("establish Doubao WebSocket TTS connection failed: websocket handshake status=%d", resp.StatusCode)
	}
	if bodyText != "" {
		return fmt.Errorf("establish Doubao WebSocket TTS connection failed: websocket handshake status=%d body=%s", resp.StatusCode, bodyText)
	}
	return fmt.Errorf("establish Doubao WebSocket TTS connection failed: websocket handshake status=%d", resp.StatusCode)
}
...
func summarizeDoubaoWSAttemptError(voice string, preferred resolvedTTSModel, attemptedResources []string, attemptErrors []error) error {
	...
	if len(attemptedResources) > 0 {
		retryInfo = fmt.Sprintf(", already tried resource_id=%s", strings.Join(attemptedResources, ","))
	}

	if hasNotGranted && hasMismatch {
		return fmt.Errorf(
			"Doubao WebSocket TTS config not available: voice=%s, model=%s, resource_id=%s%s. Current explicit resource_id is not authorized for this app/token, fallback to model-related public resource still shows voice and resource mismatch. Please check the actual voice/ResourceID mapping enabled for your AccessKey in the Volcano console",
			voice,
	...
	if hasNotGranted {
		return fmt.Errorf(
			"Doubao WebSocket TTS resource_id not authorized: voice=%s, model=%s, resource_id=%s%s. Current app/token has no permission to access this ResourceID, please use the ResourceID actually authorized for your current AccessKey, or clear resource_id to use model default mapping",
			voice,
			preferred.ConfigModel,
			preferred.ResourceID,
			retryInfo,
	...
	hint := "Selected voice and current app/token accessible v3 ResourceID do not match. Old v1 voice list will not automatically migrate to v3."
	...
	case "zh_female_wanwanxiaohe_moon_bigtts", "zh_female_qinqienvsheng_moon_bigtts":
		hint = "Selected voice is from the old static list, after upgrading to v3 usually need to switch to the voice actually enabled for your account, old data will not automatically migrate."
	}

	return fmt.Errorf(
		"Doubao WebSocket TTS voice and resource mismatch: voice=%s, model=%s, resource_id=%s%s. %s Please check the actual voice/ResourceID mapping enabled in the Volcano console or your account",
		voice,
		preferred.ConfigModel,
		preferred.ResourceID,
		retryInfo,
