...
func init() {
	// Do not set default output, let application decide
	log.SetFormatter(Formatter(false)) // Default no color
}

// SetOutput sets log output target
...
// getCaller gets actual caller info (skip logger wrapper layer)
func getCaller() (string, int) {
	// Skip log library call stack, get actual caller
	// Through call stack: user code -> logger.Info -> addCallerField -> getCaller -> runtime.Caller
	// So need to skip 3 levels to reach actual call location
...
func Log(args ...interface{}) *log.Entry {
	...
	// Add caller info
	// Also need to adjust level in Log function call chain
...
func Formatter(isConsole bool) *nested.Formatter {
	fmtter := &nested.Formatter{
		...
		ShowFullLevel:    true,
		//NoFieldsSpace:    true,
		// Disable default caller formatting since we already added custom caller field
...
// DebugStack for debugging log call stack, outputs all caller info in current call chain
func DebugStack() {
	for i := 0; i < 5; i++ {
