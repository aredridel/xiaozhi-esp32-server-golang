...
func (ac *AdminController) GetMCPMarkets(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get MCP market connection"})
		return
	}
...
func (ac *AdminController) UpdateMCPMarket(c *gin.Context) {
	...
	if err := ac.DB.Where("id = ? AND type = ?", id, mcpmarket.MCPMarketConfigType).First(&existing).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "MCP market connection does not exist"})
		return
	}
...
func (ac *AdminController) ImportMCPMarketService(c *gin.Context) {
	...
	if len(detail.Endpoints) == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "This service has no available SSE/StreamableHTTP addresses, please activate or deploy in the upstream market first"})
		return
	}
	...
	if len(upserts) == 0 {
		if len(skippedURLs) > 0 {
			c.JSON(http.StatusConflict, gin.H{"error": "All importable addresses conflict with manual config URLs, skipped based on manual priority", "skipped_urls": skippedURLs})
			return
		}
		c.JSON(http.StatusBadRequest, gin.H{"error": "No valid importable addresses found"})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("Failed to aggregate MCP config: %v", err)})
		return
	}
	...
	if err := tx.Commit().Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("Failed to persist imported services: %v", err)})
		return
	}
...
func (ac *AdminController) loadEnabledMarketsWithAuth() ([]marketWithAuth, error) {
	...
	for _, cfg := range configs {
		...
		if err != nil {
			return nil, fmt.Errorf("Failed to decrypt auth info for market %s: %w", cfg.Name, err)
		}
...
func fetchServiceDetail(ctx context.Context, marketModel models.Config, marketCfg mcpmarket.MarketConnection, authCfg mcpmarket.AuthConfig, serviceID string) (*mcpmarket.MarketServiceDetail, error) {
	if mcpmarket.NormalizeProviderID(marketCfg.ProviderID) == mcpmarket.ProviderModelScope && strings.TrimSpace(authCfg.Token) == "" {
		return nil, fmt.Errorf("ModelScope service details require Token, please fill in the Token in this market connection")
	}
	...
	if err != nil {
		if mcpmarket.NormalizeProviderID(marketCfg.ProviderID) == mcpmarket.ProviderModelScope {
			...
			if strings.Contains(errMsg, "invalidauthentication") || strings.Contains(errMsg, "authentication failed") || strings.Contains(errMsg, "401") {
				return nil, fmt.Errorf("ModelScope service details authentication failed, please check if the Token in the market connection is valid")
			}
		}
	...
	if mcpmarket.NormalizeProviderID(marketCfg.ProviderID) == mcpmarket.ProviderModelScope && len(detail.Endpoints) == 0 {
		return nil, fmt.Errorf("This service has no callable addresses (may not be activated on ModelScope yet), please activate on ModelScope first")
	}
	return detail, nil
}
...
func fetchMarketCatalog(ctx context.Context, marketCfg mcpmarket.MarketConnection, authCfg mcpmarket.AuthConfig) (interface{}, error) {
	...
	if mcpmarket.NormalizeProviderID(marketCfg.ProviderID) == mcpmarket.ProviderModelScope {
		if strings.TrimSpace(authCfg.Token) == "" {
			return nil, fmt.Errorf("ModelScope market only fetches activated services, please fill in the Token in this market connection first")
		}
...
func mergeEndpointsIntoMCPConfig(current mcpmarket.MergedMCPConfig, detail *mcpmarket.MarketServiceDetail, nameOverride string) (mcpmarket.MergedMCPConfig, []string, string, error) {
	...
	if err != nil {
		return mcpmarket.MergedMCPConfig{}, nil, "", fmt.Errorf("Failed to parse existing MCP services: %w", err)
	}
	...
	for idx, endpoint := range detail.Endpoints {
		...
		if _, exists := existingURLSet[normURL]; exists {
			return mcpmarket.MergedMCPConfig{}, nil, endpoint.URL, fmt.Errorf("URL conflict, same MCP service address already exists")
		}
...
func buildStoredMarketConfig(req upsertMCPMarketRequest, existing *mcpmarket.MarketConnection) (mcpmarket.MarketConnection, error) {
	...
	if _, err := url.ParseRequestURI(catalogURL); err != nil {
		return mcpmarket.MarketConnection{}, fmt.Errorf("catalog_url format is incorrect")
	}
	if detailTemplate != "" {
		...
		if _, err := url.ParseRequestURI(validateURL); err != nil {
			return mcpmarket.MarketConnection{}, fmt.Errorf("detail_url_template format is incorrect")
		}
	}
	...
	case existing == nil && authType != mcpmarket.AuthTypeNone:
		return mcpmarket.MarketConnection{}, fmt.Errorf("Authentication is enabled but no token provided")
	}
	...
	if req.Auth.ExtraHeaders != nil {
		...
		if err != nil {
			return mcpmarket.MarketConnection{}, fmt.Errorf("extra_headers format error")
		}
...
func decodeMarketAuthConfig(cfg mcpmarket.MarketConnection) (mcpmarket.AuthConfig, error) {
	...
	if cfg.TokenCiphertext != "" {
		...
		// Compatible with historical encrypted storage: if old key is configured, decrypt and override
	...
	if cfg.ExtraHeadersCiphertext != "" {
		...
		if err := json.Unmarshal([]byte(headersJSON), &headers); err != nil {
			// Compatible with historical encrypted storage: try to decrypt using old key
