...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if authHeader == "" {
			log.Printf("[JWTAuth] ❌ Missing auth header")
		...
		if err != nil {
			log.Printf("[JWTAuth] ❌ Token parsing failed: %v", err)
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Invalid token"})
			c.Abort()
			return
		}

		log.Printf("[JWTAuth] ✅ Token verified - User ID: %d, Username: %s, Role: %s", claims.UserID, claims.Username, claims.Role)
...
// AdminAuth admin permission middleware
