...
// OpenAPIAuth supports JWT or API Token authentication.
// API Token supports two request headers:
...
func OpenAPIAuth(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if rawToken == "" {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Missing authentication info (JWT or API Token)"})
			c.Abort()
			return
		}
		...
		if err := db.First(&user, apiToken.UserID).Error; err != nil {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "API Token owner user does not exist"})
			c.Abort()
			return
		}
