...
type Config struct {
	...
	// Connection pool configuration (SQLite typically needs only single connection)
	MaxIdleConns    int `json:"max_idle_conns"`
	MaxOpenConns    int `json:"max_open_conns"`
	ConnMaxLifetime int `json:"conn_max_lifetime"`
}

// NewConfigFromDatabase creates SQLite configuration from database configuration
...
func (c *Config) DSN() string {
	// Ensure using file: prefix to support more options
