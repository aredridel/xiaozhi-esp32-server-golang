...
// StorageAdapter storage adapter for bridging interface differences
...
// Connect connects to database (adapter method)
func (a *StorageAdapter) Connect(ctx context.Context) error {
	// Base class already connected, this is just interface adaptation
	return nil
}

// Ping checks database connection (adapter method)
func (a *StorageAdapter) Ping(ctx context.Context) error {
	return a.GormBaseStorage.Ping()
}

// UserStorage returns user storage interface
...
// DeviceStorage returns device storage interface
...
// GetDeviceByCode gets a device by device code
...
// GetDevicesByUserID gets device list by user ID
...
// AgentStorage returns agent storage interface
...
// GetAgentsByUserID gets agent list by user ID
...
// ConfigStorage returns configuration storage interface
