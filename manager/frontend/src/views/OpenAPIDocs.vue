<template>
  <div class="vp-docs">
    <aside class="vp-sidebar">
      <div class="vp-sidebar-title">OpenAPI Docs</div>
      <a v-for="item in nav" :key="item.id" :href="`#${item.id}`" class="vp-nav-item">{{ item.label }}</a>
    </aside>

    <main class="vp-content">
      <header class="vp-hero">
        <h1>Xiaozhi OpenAPI Docs</h1>
        <p class="lead">Publicly accessible. Provides request method, parameters, response and examples for each endpoint.</p>
        <div class="hero-meta">
...
X-API-Token: &lt;api-token&gt;</code></pre>
      ...
      <section id="common" class="vp-section">
        <h2>General Response Info</h2>
        <ul>
          <li>Common error codes: <code>400</code> Bad request, <code>401</code> Authentication failed, <code>404</code> Resource not found, <code>500</code> Server error.</li>
          <li>Paginated endpoints default: <code>page=1</code>, <code>page_size=50</code>.</li>
        </ul>
      </section>
      ...
      <section id="profile" class="vp-section">
        ...
        <h4>Parameters</h4><p>None (authentication header only).</p>
  ...
  "user": {"id": 1, "username": "demo", "email": "demo@example.com", "role": "user"}
      ...
      <section id="agents" class="vp-section">
        ...
        <table><thead><tr><th>Field</th><th>Type</th><th>Required</th><th>Description</th></tr></thead><tbody>
          ...
          <tr><td>llm_config_id</td><td>string</td><td>No</td><td>LLM config ID</td></tr>
          <tr><td>tts_config_id</td><td>string</td><td>No</td><td>TTS config ID</td></tr>
        ...
        <table><thead><tr><th>Field</th><th>Type</th><th>Required</th><th>Description</th></tr></thead><tbody>
          <tr><td>name</td><td>string</td><td>Yes</td><td>Name, 2-50 characters</td></tr>
          <tr><td>custom_prompt</td><td>string</td><td>No</td><td>Prompt</td></tr>
          <tr><td>llm_config_id</td><td>string</td><td>No</td><td>LLM config ID (can be emptied)</td></tr>
          <tr><td>tts_config_id</td><td>string</td><td>No</td><td>TTS config ID (can be emptied)</td></tr>
          <tr><td>voice</td><td>string</td><td>No</td><td>Voice identifier</td></tr>
          <tr><td>asr_speed</td><td>string</td><td>No</td><td>Empty defaults to normal</td></tr>
          <tr><td>memory_mode</td><td>string</td><td>No</td><td>short/long/none</td></tr>
        </tbody></table>
        <h4>Response Example</h4>
      ...
      <section id="mcp" class="vp-section">
        <h2>6. MCP Tools API</h2>

        <h3>6.1 Get Agent Tool List</h3>
        ...
        <h3>6.3 Get Device Tool List</h3>
        <div class="api-line"><span class="method get">GET</span><code>/api/open/v1/devices/:id/mcp-tools</code></div>
        <h4>Path Parameters</h4>
        <table><thead><tr><th>Parameter</th><th>Type</th><th>Required</th><th>Description</th></tr></thead><tbody>
          <tr><td>id</td><td>number</td><td>Yes</td><td>Device ID</td></tr>
        </tbody></table>
        <h4>Notes</h4>
        <ul>
          <li>Only returns device-side IoT over MCP tools for the currently online transport.</li>
          <li>Does not mix in historical tools from other transports or agent ws-endpoint tools.</li>
        </ul>
        <h4>Response Example</h4>
        <pre><code>{"data":{"tools":[{"name":"device_tool","description":"..."}]}}</code></pre>

        <h3>6.4 Call Device Tool</h3>
        ...
        <h4>Notes</h4>
        <ul>
          <li>Prioritizes matching against the current device tool list.</li>
          <li>When a tool is not yet in the list but the current runtime is still available, the server will automatically attempt a raw call fallback.</li>
        </ul>
        <h4>Response Example</h4>
...
const nav = [
  ...
  { id: 'common', label: 'General Info' },
  { id: 'profile', label: '1. User Info' },
  { id: 'devices', label: '2. Device API' },
  { id: 'agents', label: '3. Agent API' },
