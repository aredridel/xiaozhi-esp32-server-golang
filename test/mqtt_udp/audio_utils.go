...
// Returns a slice collection of Opus frames, each slice is an Opus encoded frame
...
func OpusToWav(opusData [][]byte, sampleRate int, channels int, fileName string) ([][]int16, error) {
	...
	wavBuffer := audio.IntBuffer{
		Format: &audio.Format{
			NumChannels: channels, // Use the passed channel count
